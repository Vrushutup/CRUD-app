...
exports.create=(req,res)=>{
    ...
    .catch(err=>{
        res.status(500).send({
            message:err.message||"Some error ocuured"
        })
    })
}
...
.catch(err=>{
    res.status(500).send({message:"Error retrieving with user id "+id})
})
}   
else{
    Userdb.find()
.then(user=>{
    ...
    res.status(500).send({message:err.message||'error ocuured while retrieving user info'})
})
}
}
